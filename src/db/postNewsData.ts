...
const qInsert = () =>
  `INSERT INTO ${NEWS_TABLE}(headline, provider, url, timestamp) VALUES (
      $1, 
      $2, $3, to_timestamp($4)) RETURNING *;`;
...
export const postNewsData = async (pool: Pool, data: Headlines) => {
  ...
  console.log(`Added ${items.length} items to ${NEWS_TABLE} table in ${DB_NAME} data base. ${duplicateCount} duplicate url's omitted.`);
  return items.flat();
};
